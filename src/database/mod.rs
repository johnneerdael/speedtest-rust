...
pub fn init() -> std::io::Result<Arc<Mutex<dyn Database + Send>>> {
    ...
    match config.database_type.as_str() {
        ...
        "mysql" => {
            ...
            Ok(Arc::new(Mutex::new(MySql {
                connection: mysql_setup,
            })))
        }
        #[cfg(feature = "postgres")]
        "postgres" => {
            let postgres_setup = postgres::init(
                &config.database_username,
                &config.database_password,
                &config.database_hostname,
                &config.database_name,
            ...
            Ok(Arc::new(Mutex::new(Postgres {
                connection: postgres_setup,
            })))
        }
        #[cfg(feature = "sqlite")]
        "sqlite" => {
        ...
        _ => Err(Error::other("Invalid database type.")),
    }
}
