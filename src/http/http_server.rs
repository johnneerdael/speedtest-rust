...
use log::{info, trace};
use std::sync::Arc;
use tokio::io::{split, AsyncReadExt, AsyncWriteExt, BufReader, BufWriter};
...
impl HttpServer {
    pub async fn init() -> std::io::Result<Self> {
        ...
        if config.enable_tls {
            tls_acceptor = Some(setup_tls_acceptor(
                        ...
                        if let Some(tls_acceptor) = tls_acceptor {
                            ...
                            match stream {
                                Ok(stream) => {
                                    ...
                                    let mut buff_reader =
                                        BufReader::with_capacity(8 * 1024, socket_r);
                                    let mut buff_writer =
                                        BufWriter::with_capacity(8 * 1024, socket_w);
            ...
            Box::pin(async move {
                if let Some(route) = ROUTES.get().unwrap().get(request.path.trim()) {
                    match *route {
                        ...
                        "results" => show_result_route(&mut database, &request.query_params).await,
                        "results/telemetry" => {
                            telemetry_record_route(&mut database, &request).await
                        }
                        "stats" => handle_stat_page(&request, &mut database).await,
                        _ => Response::res_404(),
                    }
